...
function parseArguments(array $argv): array
{
    $options = [
        ...
        'quiet' => false,
        'verbose' => false,
        'maxThreads' => 4, // Default to 4 parallel threads
    ];
    ...
    for ($i = 1; $i < count($argv); $i++) {
        ...
        } elseif (str_starts_with($arg, '--max-threads=')) {
            $options['maxThreads'] = (int) substr($arg, 14);
...
function initializeTestRunner(): callable
{
    return function (object $t): void {
        ...
        if (!$options['quiet']) {
            echo "\n";
            echo "========================================\n";
            echo " PARALLEL TEST RUNNER - " . strtoupper($spec['name'] ?? 'ACCEPTANCE CRITERIA') . "\n";
            echo "========================================\n";
            echo "\n";
            ...
            echo "  Max parallel threads: {$options['maxThreads']}\n";
...
/**
 * Run a single feature's tests (used by parallel execution)
 */
...
/**
 * Run all tests in parallel - one process per feature
 */
function runAllTestsParallel(): callable
{
    return function (object $t): void {
        $features = $this->get('features');
        $quiet = $this->get('quiet', false);
        ...
        $maxThreads = $this->get('maxThreads', 4);

        if (!$quiet) {
            echo "\n";
            echo "Running tests in parallel (max {$maxThreads} threads)...\n";
            echo "========================================\n";
        }

        // If only one feature, run sequentially
        if (count($features) === 1) {
            if (!$quiet) {
                echo "Only one feature detected, running sequentially...\n";
            }
            $callback = runAllTestsSequential();
            $callback->call($this, $t);
            return;
        }

        // Run features in parallel batches
        $featureBatches = array_chunk($features, $maxThreads);
        $allResults = [];

        foreach ($featureBatches as $batchIndex => $batch) {
            $processes = [];
            $pipes = [];

            // Start processes for this batch
            foreach ($batch as $featureIndex => $feature) {
                if (!$quiet) {
                    echo "Starting feature: {$feature['name']} (" . count($feature['specs']) . " tests)\n";
                }

                // Create a temporary PHP script to run this feature's tests
                $tempScript = tempnam(sys_get_temp_dir(), 'feature_test_');

                // Encode data to pass via file instead of embedding in script
                $dataFile = tempnam(sys_get_temp_dir(), 'feature_data_');
...
PHP;
                ...
                $process = proc_open(
                    "php " . escapeshellarg($tempScript) . " " . escapeshellarg($dataFile),
                    $descriptorspec,
                    $processPipes,
                    '/var/www/html'
                );
                ...
                if (is_resource($process)) {
                    $processes[$featureIndex] = [
                        'process' => $process,
                        'feature' => $feature,
                        'tempScript' => $tempScript,
                        'dataFile' => $dataFile,
                    ];
                    $pipes[$featureIndex] = $processPipes;
                }
            }

            // Collect results from all processes in this batch
            foreach ($processes as $featureIndex => $processData) {
                $process = $processData['process'];
                $feature = $processData['feature'];
                $tempScript = $processData['tempScript'];
                $processPipes = $pipes[$featureIndex];
                ...
                // Read stderr
                $errors = stream_get_contents($processPipes[2]);
                ...
                if ($results === null) {
                    // Process failed to return valid JSON
                    echo "❌ ERROR: Feature '{$feature['name']}' failed to execute properly\n";
                    if (!empty($errors)) {
                        echo "Error output: {$errors}\n";
                    }
                    echo "Raw output: {$output}\n";

                    // Create failure results for all specs in this feature
                    $results = [];
                    foreach ($feature['specs'] as $spec) {
                        $results[] = [
                            'feature' => $feature['name'],
                ...
                if (!$quiet) {
                    ...
                    $icon = count($failed) === 0 ? '✅' : '❌';
                    echo "{$icon} Completed: {$feature['name']} ({" . count($passed) . "/" . count($results) . "} passed)\n";
                }
            }
        }
        ...
        foreach ($allResults as $result) {
            ...
            if ($result['passed']) {
                ...
                echo "❌ FAILED: [{$result['feature']}] {$result['acceptanceCriteria']}\n";
...
/**
 * Fallback to sequential execution (for single feature or when parallel fails)
 */
function runAllTestsSequential(): callable
{
    return function (object $t): void {
        $features = $this->get('features');
        $quiet = $this->get('quiet', false);
