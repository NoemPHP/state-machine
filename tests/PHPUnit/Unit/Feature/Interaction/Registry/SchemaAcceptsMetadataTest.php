...
class SchemaAcceptsMetadataTest extends TestCase
{
    public function testSchemaValidatesMetadataAsOptionalArrayInInteractionDefinitions(): void
    {
        $yamlContent = <<<YAML
states:
  - name: ready
...
YAML;
        ...
        try {
            ...
            $definition = $registry->get('confirm_action');

            $this->assertNotNull($definition);
            $this->assertSame(['importance' => 'high', 'category' => 'critical'], $definition->metadata);
        } finally {
            unlink($tempFile);
        }
    }
