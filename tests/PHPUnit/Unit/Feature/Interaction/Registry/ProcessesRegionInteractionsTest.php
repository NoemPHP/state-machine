...
class ProcessesRegionInteractionsTest extends TestCase
{
    public function testEnhanceRegionBuilderProcessesRegionLevelInteractionsDuringBuildPhase(): void
    {
        $yamlContent = <<<YAML
interactions:
  - id: global_help
...
YAML;
        ...
        try {
            ...
            $definition = $registry->get('global_help');

            $this->assertNotNull($definition);
            $this->assertSame('global_help', $definition->id);
            $this->assertSame('confirm', $definition->type);
            $this->assertSame('help', $definition->state);
        } finally {
            unlink($tempFile);
        }
    }
