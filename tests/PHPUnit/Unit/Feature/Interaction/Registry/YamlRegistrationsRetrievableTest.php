...
class YamlRegistrationsRetrievableTest extends TestCase
{
    public function testYamlRegisteredInteractionsRetrievableViaInteractionRegistryGet(): void
    {
        $yamlContent = <<<YAML
states:
  - name: ready
...
YAML;
        ...
        try {
            ...
            $definition = $registry->get('proceed_confirm');

            $this->assertNotNull($definition);
            $this->assertSame('proceed_confirm', $definition->id);
            $this->assertSame('confirm', $definition->type);
            $this->assertSame('ready', $definition->state);
            $this->assertSame('Proceed with operation?', $definition->question);
        } finally {
            unlink($tempFile);
        }
    }
