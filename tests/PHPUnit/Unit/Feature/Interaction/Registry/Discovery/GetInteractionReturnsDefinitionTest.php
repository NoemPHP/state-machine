...
class GetInteractionReturnsDefinitionTest extends TestCase
{
    public function testGetInteractionAbilityReturnsSingleInteractionDefinitionWhenIdExists(): void
    {
        ...
        $result = ($ability->handler)(['id' => 'test_confirm']);

        $this->assertIsArray($result);
        $this->assertSame('test_confirm', $result['id']);
        $this->assertSame('confirm', $result['type']);
        $this->assertSame('ready', $result['state']);
        $this->assertSame('Proceed?', $result['question']);
    }
}
