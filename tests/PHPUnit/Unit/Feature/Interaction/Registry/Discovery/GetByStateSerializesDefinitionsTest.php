...
class GetByStateSerializesDefinitionsTest extends TestCase
{
    public function testGetInteractionsForStateAbilitySerializesDefinitionsToJsonCompatibleArrays(): void
    {
        ...
        $this->assertSame('ready_confirm', $result['interactions'][0]['id']);
        $this->assertSame('confirm', $result['interactions'][0]['type']);
        $this->assertSame('ready', $result['interactions'][0]['state']);
        $this->assertSame('Ready?', $result['interactions'][0]['question']);
        $this->assertSame(['defaultValue' => false], $result['interactions'][0]['metadata']);
    }
}
