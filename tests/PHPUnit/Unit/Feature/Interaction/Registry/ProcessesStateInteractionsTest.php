...
class ProcessesStateInteractionsTest extends TestCase
{
    public function testEnhanceRegionBuilderProcessesStateLevelInteractionsDuringBuildPhase(): void
    {
        $yamlContent = <<<YAML
states:
  - name: active
...
YAML;
        ...
        try {
            ...
            $definition = $registry->get('active_confirm');

            $this->assertNotNull($definition);
            $this->assertSame('active_confirm', $definition->id);
            $this->assertSame('confirm', $definition->type);
            $this->assertSame('active', $definition->state);
        } finally {
            unlink($tempFile);
        }
    }
