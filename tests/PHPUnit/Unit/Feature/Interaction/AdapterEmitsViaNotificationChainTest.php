...
/**
 * @spec Framework adapters emit InteractionResponse via source.notificationChain
 * @see specs/features/interaction.yaml - framework-adapter-subscription
 */
class AdapterEmitsViaNotificationChainTest extends TestCase
{
    public function testAdapterEmitsViaNotificationChain(): void
    {
        ...
        $builder->addState('test')->onEnter('test', function (object $trigger) use (&$responseDelivered) {
                $request = new ConfirmRequest(question: 'Test?');

                $request->then(function ($response) use (&$responseDelivered) {
