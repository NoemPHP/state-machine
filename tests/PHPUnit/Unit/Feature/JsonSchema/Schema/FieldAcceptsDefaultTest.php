...
/**
 * Acceptance Criteria: Schema field definition accepts optional default value
 * Intent: Provides initial value for field when machine builds, enabling predictable context state
 * Criticality: contract
 */
final class FieldAcceptsDefaultTest extends TestCase
{
    public function testSchemaFieldAcceptsDefault(): void
    {
        ...
        // Test that field with default value passes validation
        $normalizedValid = $processor->process($regionSchema, [
            'name' => 'test',
            'context' => [
                'schema' => [
                    ['name' => 'field', 'type' => 'string', 'default' => 'default_value'],
                ],
            ],
        ]);

        $schema = (array)$normalizedValid->context->schema;
        $this->assertEquals('default_value', $schema[0]['default']);
    }
}
