...
/**
 * Acceptance Criteria: Schema field definition accepts optional description as string
 * Intent: Documents field purpose for developer reference and introspection
 * Criticality: contract
 */
final class FieldAcceptsDescriptionTest extends TestCase
{
    public function testSchemaFieldAcceptsDescription(): void
    {
        ...
        // Test that field with description passes validation
        $normalizedValid = $processor->process($regionSchema, [
            'name' => 'test',
            'context' => [
                'schema' => [
                    ['name' => 'field', 'type' => 'string', 'description' => 'A descriptive field'],
                ],
            ],
        ]);

        $schema = (array)$normalizedValid->context->schema;
        $this->assertEquals('A descriptive field', $schema[0]['description']);
    }
}
