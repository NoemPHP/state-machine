 ...
 */
final class TrueExposesTest extends RegionBuilderTestCase
{
    public function testPredicateReturningTrueExposesAbility(): void
    {
        $invoked = false;
        ...
        $region = (new RegionBuilder())
            ...
            ->setStates('idle')
            ->onEnter('idle', function (object $t) use ($predicate, $handler, &$invoked) {
                ...
                $this->abilities('conditional-ability');
            })
            ->build();

        $this->assertTrue(
            $invoked,
            'Ability with predicate returning true must be invocable'
        );
    }
}
