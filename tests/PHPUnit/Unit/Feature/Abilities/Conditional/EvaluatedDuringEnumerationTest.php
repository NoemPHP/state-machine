 ...
 */
final class EvaluatedDuringEnumerationTest extends RegionBuilderTestCase
{
    public function testPredicateEvaluatedDuringEnumeration(): void
    {
        $predicateEvaluated = false;

        $predicate = function (Region $region) use (&$predicateEvaluated) {
        ...
        $region = (new RegionBuilder())
            ...
            ->setStates('idle')
            ->onEnter('idle', function (object $t) use ($predicate, $handler, &$predicateEvaluated) {
        ...
        $this->assertTrue(
            $predicateEvaluated,
            'Predicate must be evaluated during enumeration to filter results'
        );
    }
}
