...
/**
 * Acceptance Criterion: BoundAccess extracts parameters from second argument
 *
 * Intent: Parses invocation parameters from arguments,
 * supporting $this->abilities('name', $params) syntax
 */
#[Group('abilities')]
#[Group('bound-access-integration')]
class ExtractsParametersTest extends TestCase
{
    public function testExtractsParametersFromSecondArgument(): void
    {
        ...
        // Track which parameters were passed
        $invokedParameters = null;
        $invokeAbility->link(function (InvokeAbilityParams $params, callable $next) use (&$invokedParameters) {
            $invokedParameters = $params->parameters;
        ...
        // Create params with parameters as second argument
        $expectedParams = ['key' => 'value', 'count' => 42];
        ...
        // Then: Should have extracted parameters from second argument
        // (This assertion will be reached once middleware is implemented)
        $this->assertEquals($expectedParams, $invokedParameters);
    }
}
