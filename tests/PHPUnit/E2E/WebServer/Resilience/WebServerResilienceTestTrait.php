...
/**
 * Shared callbacks for webserver resilience tests.
 *
 * Tests accept loop resilience against socket closure timing issues.
 */
trait WebServerResilienceTestTrait
{
    /**
     * Get mock-compatible container callbacks for webserver resilience tests.
     *
    ...
    protected function getWebServerCallbacks(MockSocket $mockSocket, array $overrides = []): array
    {
        $defaults = [
            ...
            'server.starting.accept' => function (object $trigger) use ($mockSocket) {
                ...
                yield;
                while (true) {
                    // Clean up closed clients BEFORE stream_select
                    // This is the critical timing fix - we must validate resources
                    // before passing them to stream_select()
                    ...
                    // Simulate stream_select by checking for new connections
                    // In real implementation, we need to handle stream_select errors gracefully
